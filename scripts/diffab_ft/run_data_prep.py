...
class Step:
    ...
    # Optional staleness check: returns (is_stale, reason) tuple. If is_stale,
    # the step is treated as if its outputs don't exist (it will re-run).
    # Catches the case where outputs exist but were produced by an older
    # version of the underlying script (e.g. pre-dedup cluster_splits.json).
    is_stale: Callable[[], tuple[bool, str]] | None = None
    ...
    def outputs_exist(self) -> bool:
        ...
        if self.is_stale is not None:
            stale, _ = self.is_stale()
            if stale:
                return False
        return True

    def staleness_reason(self) -> str:
        if self.is_stale is None:
            return ""
        _, reason = self.is_stale()
        return reason
...
def build_steps(args: argparse.Namespace) -> list[Step]:
    ...
    steps: list[Step] = [
        ...
        Step(
            ...
            is_stale=lambda: _cluster_splits_stale(splits_json, args.dedupe_by),
...
def _cluster_splits_stale(splits_json: Path, expected_dedupe_by: str) -> tuple[bool, str]:
    """A pre-existing cluster_splits.json is stale if it was generated by
    an older cluster_split.py that didn't write the dedupe_by field, or
    if its dedupe_by doesn't match what the orchestrator was asked to use.
    """
    ...
    try:
        ...
        params = d.get("params", {})
        actual = params.get("dedupe_by")
        if actual is None:
            return True, "splits JSON pre-dates dedup support (no params.dedupe_by)"
        if actual != expected_dedupe_by:
            return True, (f"splits used dedupe_by={actual!r}, "
                          f"but orchestrator wants {expected_dedupe_by!r}")
        return False, ""
    except Exception as exc:  # noqa: BLE001
        return True, f"could not parse splits JSON: {exc}"
...
def run_step(step: Step, args: argparse.Namespace) -> None:
    ...
    if step.outputs_exist() and not force:
        ...
        if step.audit:
            step.audit()
        return
    elif step.is_stale is not None and all(p.exists() for p in step.output_paths):
        # Outputs exist but staleness check says they're not usable.
        reason = step.staleness_reason()
        log.warning("  outputs exist but are stale; re-running. reason: %s", reason)
